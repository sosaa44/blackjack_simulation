...
# Gibt 1 (Gewinn) oder 0 (Nicht-Gewinn) zurück — Bernoulli-Versuch
# Unentschieden zählt als Nicht-Gewinn (0)
play_round <- function(strategy = "basic") {
  ...
  if (strategy == "basic") {
    # Basic: hit unter 17, stand ab 17
    while (player_sum < 17) {
      player_hand <- c(player_hand, sample(deck, 1))
      player_sum  <- calculate_sum(player_hand)
    }
  } else if (strategy == "martingale") {
    # Martingale: gleiche Hit/Stand-Logik wie Basic
    # Einsatzverdopplung wird in simulate_player() gehandhabt
    while (player_sum < 17) {
      player_hand <- c(player_hand, sample(deck, 1))
      player_sum  <- calculate_sum(player_hand)
    }
...
```{r}
# Jeder Spieler spielt genau 200 Runden (fixe Rundenzahl)
